...
const UtilizationLayout = () => {
  const { data, isLoading, error, filters, filterOptions, handleFetchUtilizationData, handleClearError, handleSetFilters, handleFetchFilterOptions } =
    useUtilization()
  ...
  const filteredRows = rows.filter(row => {
    if (filters.vendor && filters.vendor !== '' && row.vendor !== filters.vendor) {
      return false;
    }

    if (filters.contract && filters.contract !== '' && row.contractTitle !== filters.contract) {
      return false;
    }

    if (filters.status && filters.status !== '' && row.status !== filters.status) {
      return false;
    }
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-primary-foreground">Utilization</h1>
      </div>
